...
v0.40.0 (`0880b4d37`). These ports keep consensus sign/state bytes byte-compatible with the
...
section becomes `## v0.40.0-bera.N`._
...
- `[consensus,types,state]` Port Proposer-Based Timestamps (PBTS) from the bera-v1.x line.
  ...
  and a node panics when asked to make or validate a block at a height where it is not (as on
  bera-v1.x). Without vote timestamps the `quorum_prevote_delay`, `quorum_precommit_delay` and
...
- `[abci,state]` Add `syncing_to_height` (field 9) to `RequestFinalizeBlock`, the upstream v1 /
  ...
  highest peer height, consensus and adaptive-sync ingestion pass the block height, and handshake
  replay passes the height of the last stored block.
- `[test]` Add `test/compat`: golden vectors generated by the bera-v1.x code (sign bytes,
  signatures, hashes, persisted state, p2p/WAL wire format, key and genesis files) asserted
  byte-for-byte against this fork in both directions, plus a live data-dir reuse and
  mixed-validator rolling-upgrade script against a bera-v1.x binary.
...
- `[types,state]` New consensus parameters (`SynchronyParams`, `FeatureParams`) and the removal of
  ...
  encoding is not compatible with pristine upstream v0.40.x state.
  ([\#51](https://github.com/berachain/cometbft/pull/51))
