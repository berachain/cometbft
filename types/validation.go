...
// ErrInvalidAggCommit is returned when a commit carries aggregated signature
// flags but the validator set is not made of BLS12-381 keys.
...
// isAggregatedCommit returns true if the commit is an aggregated one.
...
// verifyAggregatedCommit verifies an aggregated commit, in which the
// individual BLS signatures have been collapsed into (at most) one aggregated
// signature for the block and one for nil.
//
// The aggregated signature is carried by the first CommitSig with
// BlockIDFlagAggCommit (respectively BlockIDFlagAggNil); the remaining
// signers for the same message carry BlockIDFlagAggCommitAbsent
// (BlockIDFlagAggNilAbsent) with an empty signature.
//
// If lookUpByIndex is true the commit signatures must have a 1-to-1
// correspondence with the validator set (and the validator address is checked
// against the commit entry); otherwise validators are looked up by address.
