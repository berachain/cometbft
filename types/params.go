...
// DefaultFeatureParams returns the default FeatureParams. PBTS is enabled
// from the first height; with BLS signature aggregation the per-vote
// timestamps that BFT Time medians over are not signed, so PBTS is the only
// viable block-time source on this fork.
...
func (params ConsensusParams) ValidateBasic() error {
	...
	if params.Feature.VoteExtensionsEnableHeight < 0 {
		return fmt.Errorf("Feature.VoteExtensionsEnableHeight cannot be negative. Got: %d", params.Feature.VoteExtensionsEnableHeight)
	}
...
func (params *ConsensusParams) ToProto() cmtproto.ConsensusParams {
	...
	// Emitted only when set, so that the serialized form of params written by
	// this fork matches what bera-v1.x wrote (it has no authority field).
	if params.Authority.Authority != "" {
		pb.Authority = &cmtproto.AuthorityParams{
